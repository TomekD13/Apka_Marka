...
export function Home() {
  ...
  return <div className="mx-auto max-w-xl">
    ...
    <section id="poznaj-boga" className="gradient-panel mt-8 rounded-2xl border p-5"><h2 className="text-xl font-bold text-slate-900 dark:text-white">{t('home.bars.studies', 'Poznaj Boga i Biblię')}</h2><p className="mt-1 text-sm text-slate-600 dark:text-slate-300">35 lekcji do samodzielnego studiowania Biblii.</p><Link to={`${base}/poznaj-boga-i-biblie`} className="mt-3 inline-flex text-sm font-semibold text-brand hover:underline dark:text-sky-300">{t('home.openStudies', 'Otwórz lekcje')} →</Link></section>
  </div>
}
